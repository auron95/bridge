...
In molti dei casi che seguono si effettuano dichiarazioni naturali, per le quali valgono i seguenti princ\^ipi.
\begin{itemize}
 \item Licitare un colore nuovo dà almeno 4 carte (almeno 5 se il compagno ha dimostrato di non averne più di 3).
 ...
 \item {\bf Per l'apertore:} dopo una risposta a livello di 1, superare strettamente 2 \j (dove \j è il colore di apertura) dà Rever, e quindi va fatto se e solo se si hanno 16+ HCP.
 Esempi di Rever: 1 \d\ - 1 \h\ - 2 \s (con \d e \s almeno quarte); 1 \h\ - 1 \sa\ - 3 \h (con \h almeno seste).
